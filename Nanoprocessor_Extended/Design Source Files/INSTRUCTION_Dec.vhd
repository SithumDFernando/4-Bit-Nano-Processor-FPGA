...
architecture Behavioral of INSTRUCTION_DEC is
    ...
    signal sig_move, sig_and, sig_neg, sig_jump, sig_sub : std_logic;
    signal sig_move2, sig_and2, sig_neg2, sig_jump2, sig_sub2 : std_logic_vector (2 downto 0);
    signal sig_move3, sig_and3, sig_neg3, sig_jump3, sig_sub3 : std_logic_vector( 3 downto 0);
    signal m_LSB, a_LSB, n_LSB, j_LSB, s_LSB, LSB0 : std_logic_vector(3 downto 0);
    signal m_Reg_EN, a_Reg_EN, n_Reg_EN, j_Reg_EN, s_Reg_EN, Reg_EN0 : std_logic_vector (2 downto 0);
    signal m_Mux_A, a_Mux_A, n_Mux_A, j_Mux_A, s_Mux_A, Mux_A0 : std_logic_vector (2 downto 0);
    signal m_LD, a_LD, n_LD, j_LD, s_LD, LD0 : std_logic;
    signal m_Mux_B, a_Mux_B, n_Mux_B, j_Mux_B, s_Mux_B, Mux_B0 : std_logic_vector ( 2 downto 0);
    signal m_Sub, a_Sub, n_Sub, j_Sub, s_Sub, Sub0 : std_logic;
    signal m_JMP, a_JMP, n_JMP, j_JMP, s_JMP, JMP0 : std_logic;
...
begin
    ...
    LSB <= (m_LSB AND sig_move3) OR (a_LSB AND sig_and3) OR (n_LSB AND sig_neg3) OR (j_LSB AND sig_jump3) OR (s_LSB AND sig_sub3);
    Reg_EN <= (m_Reg_EN AND sig_move2) OR (a_Reg_EN AND sig_and2) OR (n_Reg_EN AND sig_neg2) OR (j_Reg_EN AND sig_jump2) OR (s_Reg_EN AND sig_sub2);
    Mux_A <= (m_Mux_A AND sig_move2) OR (a_Mux_A AND sig_and2) OR (n_Mux_A AND sig_neg2) OR (j_Mux_A AND sig_jump2) OR (s_Mux_A AND sig_sub2);
    LD <= (m_LD AND sig_move) OR (a_LD AND sig_and) OR (n_LD AND sig_neg) OR (j_LD AND sig_jump) OR (s_LD AND sig_sub);
    Mux_B <= (m_Mux_B AND sig_move2) OR (a_Mux_B AND sig_and2) OR (n_Mux_B AND sig_neg2) OR (j_Mux_B AND sig_jump2) OR (s_Mux_B AND sig_sub2);
    Sub <= (m_Sub AND sig_move) OR (a_Sub AND sig_and) OR (n_Sub AND sig_neg) OR (j_Sub AND sig_jump) OR (s_Sub AND sig_sub);
    JMP <= (m_JMP AND sig_move) OR (a_JMP AND sig_and) OR (n_JMP AND sig_neg) OR (j_JMP AND sig_jump) OR (s_JMP AND sig_sub);
end Behavioral;
